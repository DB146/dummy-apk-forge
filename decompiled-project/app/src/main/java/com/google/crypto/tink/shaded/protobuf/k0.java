package com.google.crypto.tink.shaded.protobuf;

import java.lang.reflect.Field;
import sun.misc.Unsafe;

/* loaded from: classes.dex */
public abstract class k0 {

    /* renamed from: a, reason: collision with root package name */
    public final Unsafe f15793a;

    public k0(Unsafe unsafe) {
        this.f15793a = unsafe;
    }

    public final int a(Class cls) {
        return this.f15793a.arrayBaseOffset(cls);
    }

    public final int b(Class cls) {
        return this.f15793a.arrayIndexScale(cls);
    }

    public abstract boolean c(Object obj, long j);

    public abstract byte d(Object obj, long j);

    public abstract double e(Object obj, long j);

    public abstract float f(Object obj, long j);

    public final int g(Object obj, long j) {
        return this.f15793a.getInt(obj, j);
    }

    public final long h(Object obj, long j) {
        return this.f15793a.getLong(obj, j);
    }

    public final Object i(Object obj, long j) {
        return this.f15793a.getObject(obj, j);
    }

    public final long j(Field field) {
        return this.f15793a.objectFieldOffset(field);
    }

    public abstract void k(Object obj, long j, boolean z8);

    public abstract void l(Object obj, long j, byte b2);

    public abstract void m(Object obj, long j, double d10);

    public abstract void n(Object obj, long j, float f4);

    public final void o(Object obj, int i7, long j) {
        this.f15793a.putInt(obj, j, i7);
    }

    public final void p(Object obj, long j, long j10) {
        this.f15793a.putLong(obj, j, j10);
    }

    public final void q(Object obj, long j, Object obj2) {
        this.f15793a.putObject(obj, j, obj2);
    }
}
